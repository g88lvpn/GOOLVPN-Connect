...
import io.nekohasekai.sfa.goolvpn.ActivationLinkParser
...
class MainActivity :
    ...
    private lateinit var goolvpnViewModel: GoolvpnViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!Settings.goolvpnUpdateChannelInitialized) {
            Settings.checkUpdateEnabled = true
            Settings.updateCheckPrompted = true
            Settings.goolvpnUpdateChannelInitialized = true
        }
        if (!Settings.goolvpnUpdatePreloadInitialized) {
            Settings.autoUpdateEnabled = true
            Settings.goolvpnUpdatePreloadInitialized = true
        ...
        setContent {
            ...
            var themeMode by remember {
                mutableStateOf(GoolvpnThemeMode.fromPreference(Settings.goolvpnThemeMode))
            }
            SFATheme(darkTheme = themeMode.resolveDarkMode(systemDark)) {
                SFAApp(
                    themeMode = themeMode,
                    onThemeModeChange = { mode ->
                        Settings.goolvpnThemeMode = mode.preferenceValue
                        themeMode = mode
                    },
                )
            }
    ...
    private fun handleIntent(intent: Intent?) {
        ...
        if (uri.scheme == "goolvpn") {
            val activationToken = ActivationLinkParser.parse(uri.toString())
            if (activationToken == null) {
                pendingIntentErrorMessage = getString(R.string.goolvpn_activation_link_missing_code)
            } else {
                pendingActivationToken = activationToken
            }
            return
        }
    ...
    fun SFAApp(
        ...
        val goolvpnViewModel: GoolvpnViewModel = viewModel()
        if (!::dashboardViewModel.isInitialized) {
            this.dashboardViewModel = dashboardViewModel
        }
        if (!::goolvpnViewModel.isInitialized) {
            this.goolvpnViewModel = goolvpnViewModel
        }
        val activationToken = pendingActivationToken
        LaunchedEffect(activationToken) {
            if (activationToken != null) {
                goolvpnViewModel.activate(activationToken)
        ...
        val scaffoldContent: @Composable (PaddingValues) -> Unit = { paddingValues ->
            Box(
                ...
                SFANavHost(
                    ...
                    goolvpnViewModel = goolvpnViewModel,
                    themeMode = themeMode,
                ...
                val showPadFab = useNavigationRail &&
                    !isSubScreen &&
                    currentRootRoute != Screen.Dashboard.route &&
                    (showStartFab || showStatusBar)
                if (useNavigationRail) {
        ...
        CompositionLocalProvider(LocalTopBarController provides topBarController) {
            if (useNavigationRail) {
                ...
                Scaffold(
                    ...
                    topBar = topBarContent,
                    bottomBar = {
                        if (!isSubScreen && bottomNavigationScreens.size > 1) {
                            val hasUpdate by UpdateState.hasUpdate
                            NavigationBar {
