...
import io.nekohasekai.sfa.goolvpn.GoolvpnAppPreset
import io.nekohasekai.sfa.goolvpn.GoolvpnAppPresets
import io.nekohasekai.sfa.goolvpn.GoolvpnPerAppStore
...
fun PerAppProxyScreen(
    ...
    fun resolveSelectedUids(packageNames: Set<String>): Set<Int> =
        packages.mapNotNull { packageCache ->
            if (packageNames.contains(packageCache.packageName)) packageCache.uid else null
        }.toSet()
    ...
    fun saveSelectedApplications(packageNames: Set<String>) {
        Settings.perAppProxyList = packageNames
        if (goolvpnMode) {
            GoolvpnPerAppStore.save(context, packageNames)
        }
    }

    fun postSaveSelectedApplications(packageNames: Set<String>) {
        selectedPackageNames = packageNames
        selectedUids = resolveSelectedUids(packageNames)
        saveSelectedApplications(packageNames)
    }

    fun toggleSelection(packageCache: PackageCache, selected: Boolean) {
        val newSelected =
            if (selected) {
                selectedPackageNames + packageCache.packageName
            } else {
                selectedPackageNames - packageCache.packageName
            }
        if (newSelected == selectedPackageNames) return
        selectedPackageNames = newSelected
        selectedUids = resolveSelectedUids(newSelected)
        saveSelectedApplications(newSelected)
    }

    fun selectPreset(preset: GoolvpnAppPreset) {
        val installedPackages = packages
            .asSequence()
            .filterNot { it.isDisabled || it.isOffline }
            .map { it.packageName }
            .toSet()
        val suggestedPackages = GoolvpnAppPresets.installedPackageNames(preset, installedPackages)
        val addedCount = (suggestedPackages - selectedPackageNames).size
        postSaveSelectedApplications(selectedPackageNames + suggestedPackages)
        val message = if (addedCount == 0) {
            context.getString(R.string.goolvpn_app_preset_none)
        } else {
            context.getString(R.string.goolvpn_app_preset_added, addedCount)
        }
    ...
    LaunchedEffect(Unit) {
        ...
        val loadResult =
            withContext(Dispatchers.IO) {
                try {
                    ...
                    val settingsPackages = Settings.perAppProxyList.toSet()
                    val selectedPackageNames = if (goolvpnMode) {
                        GoolvpnPerAppStore.load(context)?.also { backedUpPackages ->
                            if (backedUpPackages != settingsPackages) {
                                Settings.perAppProxyList = backedUpPackages
                            }
                        } ?: settingsPackages.also { currentPackages ->
                            GoolvpnPerAppStore.save(context, currentPackages)
                        }
                    } else {
                        settingsPackages
                    }
                    LoadResult(mode, packageCaches, selectedPackageNames)
        ...
        selectedPackageNames = loadResult.selectedPackageNames
        selectedUids = loadResult.packages.mapNotNull { packageCache ->
            if (loadResult.selectedPackageNames.contains(packageCache.packageName)) {
                packageCache.uid
            } else {
                null
            }
        ...
        TopAppBar(
            ...
            actions = {
                ...
                PerAppProxyMenus(
                    ...
                    onSelectAll = {
                        val newSelected = currentPackages.map { it.packageName }.toSet()
                    ...
                    onImport = {
                        ...
                        if (packageNames.isNullOrEmpty()) {
                            ...
                            val newSelected = packageNames.toSet()
                            postSaveSelectedApplications(newSelected)
                            Toast.makeText(
                                context,
                    ...
                    onExport = {
                        clipboardText = selectedPackageNames.sorted().joinToString("\n")
                        Toast.makeText(
                            context,
    ...
    Column(
        ...
        if (goolvpnMode) {
            GoolvpnPerAppControls(
                proxyMode = proxyMode,
                serviceStatus = serviceStatus,
                presets = GoolvpnAppPresets.all,
                onModeChange = { mode ->
                    proxyMode = mode
                    Settings.perAppProxyEnabled = true
                    Settings.perAppProxyManagedMode = false
                ...
                Surface(
                    ...
                    Column(modifier = Modifier.padding(24.dp)) {
                        ...
                        Row(
                            ...
                            TextButton(
                                onClick = {
                                    val newSelected = selectedPackageNames.toMutableSet()
                                    result.apps.values.forEach {
                                        newSelected.remove(it.packageName)
                                    }
                                    postSaveSelectedApplications(newSelected)
                                    scanResult = null
                                },
...
@Composable
private fun GoolvpnPerAppControls(
    proxyMode: Int,
    serviceStatus: Status,
    presets: List<GoolvpnAppPreset>,
    onModeChange: (Int) -> Unit,
    onApplyPreset: (GoolvpnAppPreset) -> Unit,
) {
    ...
    Surface(
        ...
        Column(modifier = Modifier.padding(horizontal = 16.dp, vertical = 12.dp)) {
            Text(
                text = stringResource(R.string.goolvpn_app_routing_title),
            ...
            SingleChoiceSegmentedButtonRow(modifier = Modifier.fillMaxWidth()) {
                SegmentedButton(
                    selected = proxyMode == Settings.PER_APP_PROXY_EXCLUDE,
                    onClick = { onModeChange(Settings.PER_APP_PROXY_EXCLUDE) },
                    shape = SegmentedButtonDefaults.itemShape(0, 2),
                ) {
                    Text(stringResource(R.string.goolvpn_app_mode_exclude))
                }
                SegmentedButton(
                    selected = proxyMode == Settings.PER_APP_PROXY_INCLUDE,
                    onClick = { onModeChange(Settings.PER_APP_PROXY_INCLUDE) },
                    shape = SegmentedButtonDefaults.itemShape(1, 2),
                ) {
                    Text(stringResource(R.string.goolvpn_app_mode_include))
                }
            }
            Spacer(modifier = Modifier.height(8.dp))
            Text(
                text = stringResource(
                    if (proxyMode == Settings.PER_APP_PROXY_INCLUDE) {
                        R.string.goolvpn_app_mode_include_description
                    } else {
                        R.string.goolvpn_app_mode_exclude_description
                    },
                ),
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
                ...
                Text(
                    text = stringResource(
                        if (serviceStatus == Status.Started) {
                            R.string.goolvpn_app_reconnect_active
                        } else {
                            R.string.goolvpn_app_reconnect_inactive
                        },
                    ),
                    style = MaterialTheme.typography.bodySmall,
                    color = if (serviceStatus == Status.Started) {
            ...
            if (proxyMode == Settings.PER_APP_PROXY_EXCLUDE) {
                ...
                OutlinedButton(
                    ...
                    Text(stringResource(R.string.goolvpn_app_presets))
                    Spacer(modifier = Modifier.weight(1f))
                    Icon(
                        imageVector = if (presetsExpanded) Icons.Default.ExpandLess else Icons.Default.ExpandMore,
                ...
                AnimatedVisibility(
                    ...
                    Column {
                        Text(
                            text = stringResource(R.string.goolvpn_app_presets_description),
                            style = MaterialTheme.typography.bodySmall,
                            color = MaterialTheme.colorScheme.onSurfaceVariant,
                        ...
                        presets.forEach { preset ->
                            OutlinedButton(
                                ...
                                Column(modifier = Modifier.fillMaxWidth()) {
                                    ...
                                    Text(
                                        text = stringResource(preset.examplesRes),
                                        style = MaterialTheme.typography.bodySmall,
                                        color = MaterialTheme.colorScheme.onSurfaceVariant,
                                    )
                                }
