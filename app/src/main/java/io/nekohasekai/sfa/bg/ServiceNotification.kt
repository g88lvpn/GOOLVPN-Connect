...
class ServiceNotification(private val status: MutableLiveData<Status>, private val service: Service) :
    ...
    fun show(lastProfileName: String, @StringRes contentTextId: Int) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            Application.notification.createNotificationChannel(
                NotificationChannel(
                    notificationChannel,
                    service.getString(R.string.service_notification_channel_name),
    ...
    suspend fun refreshDynamicMode() {
    ...
    private fun showStaticStatus() {
        val contentTextId =
            if (status.value == Status.Started) R.string.status_started else R.string.status_starting
        Application.notificationManager.notify(
            notificationId,
            notificationBuilder.setContentText(service.getString(contentTextId)).build(),
        )
    }
