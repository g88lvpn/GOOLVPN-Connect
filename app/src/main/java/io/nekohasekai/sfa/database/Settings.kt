...
object Settings {
    ...
    var goolvpnUpdateChannelInitialized by dataStore.boolean(SettingsKey.GOOLVPN_UPDATE_CHANNEL_INITIALIZED) { false }
    var goolvpnUpdatePreloadInitialized by dataStore.boolean(SettingsKey.GOOLVPN_UPDATE_PRELOAD_INITIALIZED) { false }
    ...
    var dynamicNotification by dataStore.boolean(SettingsKey.DYNAMIC_NOTIFICATION) { false }
    var disableDeprecatedWarnings by dataStore.boolean(SettingsKey.DISABLE_DEPRECATED_WARNINGS) { false }
    var goolvpnThemeMode by dataStore.string(SettingsKey.GOOLVPN_THEME_MODE) { "system" }
    ...
    var updateReadyNotifiedVersion by dataStore.int(SettingsKey.UPDATE_READY_NOTIFIED_VERSION) { 0 }
