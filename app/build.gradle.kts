...
fun getProps(propName: String): String {
    ...
    val externalSigningFile = System.getenv("GOOLVPN_SIGNING_PROPERTIES")
        ?.takeIf { it.isNotBlank() }
        ?.let(::File)
        ?: File(System.getProperty("user.home"), "GOOLVPN-signing/signing.properties")
    for (propsFile in listOf(externalSigningFile, rootProject.file("local.properties"))) {
        if (propsFile.exists()) {
            val props = Properties()
...
val releaseKeystorePath = getProps("KEYSTORE_FILE")
...
android {
    ...
    defaultConfig {
        applicationId = "site.goolv.connect"
        minSdk = 21
        targetSdk = 35
        versionCode = getVersionProps("VERSION_CODE").toInt()
        versionName = getVersionProps("VERSION_NAME")
        base.archivesName.set("GOOLVPN-Connect-${versionName}")
        buildConfigField("String", "GOOLVPN_API_URL", "\"https://sub.goolv.site\"")
    }
...
tasks.matching { task ->
    ...
    doFirst {
        check(hasReleaseSigning) {
            "Release signing is not configured. Run scripts/create_release_key.ps1 first."
        }
        check(rootProject.file(releaseKeystorePath).isFile) {
